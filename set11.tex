...
\begin{problem}\label{inverse-square-flux}%
  Define $G : \R^3 \setminus \{0\} \to \R^3$ by
  \[
    G(x,y,z) = \frac{(x,y,z)}{(x^2+y^2+z^2)^{3/2}}.
  \]
  Compute $\operatorname{div} G$ on $\R^3 \setminus \{0\}$ and~$\displaystyle\iint_{S^2} G \cdot \hat{n}\,dS$. (cf.~\ref{solid-angle-form}).
\end{problem}
...
\begin{problem}\label{curl-grad-zero}%
  ...
  What is the relationship between this result, \ref{div-curl-zero}, and the statement $d \circ d = 0$?
\end{problem}
...
\begin{problem}
  ...
  \]
  Use this to compute the volume of the region enclosed by the torus from \ref{torus-area}.
\end{problem}
...
\begin{problem}
  If $u:\R^3\to\R$ is harmonic and bounded, then $u$ is constant.
\end{problem}
